...
void createframegrabber()
{
            ...
            ->onNewFrame([&](const SL::Screen_Capture::Image &img, const SL::Screen_Capture::Monitor &monitor) {
             ...
             //   unsigned int size = height * width * sizeof(SL::Screen_Capture::ImageBGRA);
               // auto imgbuffer(std::make_unique<unsigned char[]>(size));
                ...
                // }
               // onNewFramecounter += 1;
                printf("Capturing frame number: %d\n",++onNewFramecounter);
            })
    ...
    framgrabber->setFrameChangeInterval(std::chrono::milliseconds(50));
    framgrabber->setMouseChangeInterval(std::chrono::milliseconds(50));
}
...
int main(int argc, char** argv)
{
    ...
    // }
    int time = 5;
    // printf("Running display capturing for %d seconds",time);
    // createframegrabber();
    ...
    // printf("Pausing for %d seconds",time);
    // framgrabber->pause();
    // auto counti = 0;
    // while (counti++ < time) {
    ...
    // printf("Resuming for %d seconds",5);
    // framgrabber->resume();
